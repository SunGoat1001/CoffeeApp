<section>
    @if (!auth()->user()->hasEnabledTwoFactorAuthentication())
        @if (session('status') == 'two-factor-authentication-enabled')
            <form method="POST" action="{{ route('two-factor.confirm') }}" class="grid grid-cols-1 gap-6">
                ...
                <x-text-input type="text" name="code" :label="__('Code')" />
    ...
    @else
        ...
        <div class="flex flex-col mb-4 text-on-surface-600">
            ...
            <span class="text-sm">When two-factor authentication is enabled, you will be prompted for a secure, random
        ...
        <div>
            <details class="text-on-surface-500 mt-2 mb-4 text-sm font-bold">
                ...
                <div>
                    <span class="text-on-surface-600 text-sm font-normal">Store these recovery codes in a secure
                        password manager. They can be used to recover access to your account if your two-factor
                        authentication device is lost.</span>
